...
// ★ 삭제 권한 판단 (작성자 본인 OR admin)
const canDeletePost = (post) => {
  if (!currentUser.value) return false;
  return currentUser.value.email === post.author_email || currentUser.value.role === 'admin';
};
...
const handleDeleteOne = async (postId) => {
  ...
  try {
    ...
    alert(err.response?.data?.detail || '삭제 권한이 없습니다.');
  }
};
